...
namespace PhoenixWP\GmDhlMcFix\Currency;
 ...
 */
final class Multicurrency_Price_Converter {

	/**
	 * Returns the active storefront currency code (WCML-aware).
	 */
	public static function client_currency(): string {
		// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- WCML core filter.
		...
		$pgmdhlmc_wcml = $GLOBALS['woocommerce_wpml'] ?? null;

		if ( is_object( $pgmdhlmc_wcml ) && isset( $pgmdhlmc_wcml->multi_currency ) ) {
			$client = $pgmdhlmc_wcml->multi_currency->get_client_currency();
		...
		return strtoupper( phoenix_gm_dhl_mc_fix_base_currency() );
	}
	...
	public static function to_active_currency( float $amount_eur, string $to_currency = '' ): float {
		...
		$base = strtoupper( phoenix_gm_dhl_mc_fix_base_currency() );
